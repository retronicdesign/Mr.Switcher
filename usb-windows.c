...
int usbListDevice(HWND hVENDOR, HWND hDEVICE, HWND hVID, HWND hPID, int vendor, int product)
{
GUID                                hidGuid;        /* GUID for HID driver */
HDEVINFO                            deviceInfoList;
SP_DEVICE_INTERFACE_DATA            deviceInfo;
...
//char listitem[512];
    ...
    for(i=0;;i++){
        ...
        convertUniToAscii(buffer);
        //strcpy(listitem,buffer);
		SendMessage(hVENDOR,LB_ADDSTRING,0,(LPARAM)buffer);
        if(!HidD_GetProductString(handle, buffer, sizeof(buffer))){
            continue;
        }
        convertUniToAscii(buffer);
		//strcat(listitem,"/");
		//strcat(listitem,buffer);
		SendMessage(hDEVICE,LB_ADDSTRING,0,(LPARAM)buffer);
		//strcat(listitem,"/");
		sprintf(buffer,"%04X",deviceAttributes.VendorID);
		SendMessage(hVID,LB_ADDSTRING,0,(LPARAM)buffer);
		sprintf(buffer,"%04X",deviceAttributes.ProductID);
		SendMessage(hPID,LB_ADDSTRING,0,(LPARAM)buffer);
		//strcat(listitem,buffer);
		//strcat(listitem,"/");
		//strcat(listitem,deviceDetails->DevicePath);
		//SendMessage(hWnd,LB_ADDSTRING,0,(LPARAM)listitem);
    }
    SetupDiDestroyDeviceInfoList(deviceInfoList);
    if(deviceDetails != NULL)
        free(deviceDetails);
